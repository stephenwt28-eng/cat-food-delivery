...
const ReviewPlan = () => {
  ...
  const confirmed = window.confirm('Are you sure you want to delete this plan? This cannot be undone.');
  if (confirmed) {
    localStorage.removeItem('cart');
    localStorage.removeItem('lastOrder');
    alert('Plan deleted successfully');
    navigate('/logged-in-landing'); // or '/returning-member' depending on your flow
  }
};
  ...
  useEffect(() => {
     const savedCart = localStorage.getItem('lastOrder');
     if (!savedCart) return;

  const cart = JSON.parse(savedCart);

  // Load meats: map names to indices
  const meatIndices = cart.meats?.map(item => 
    meats.findIndex(m => m.name === item.name)
  ).filter(idx => idx !== -1) || [];
  setSelectedMeats(meatIndices);

  // Load veggies
  if (cart.veggies?.length > 0) {
    const veggieIndices = cart.veggies.map(item => 
      veggies.findIndex(v => v.name === item.name)
    ).filter(idx => idx !== -1);
    setSelectedVeggies(veggieIndices);
    setSkippedVeggies(false);
  } else {
    setSelectedVeggies([]);
    setSkippedVeggies(true);
  }

  // Load supplements
  if (cart.supplements?.length > 0) {
    const suppIndices = cart.supplements.map(item => 
      supplements.findIndex(s => s.name === item.name)
    ).filter(idx => idx !== -1);
    setSelectedSupplements(suppIndices);
    setSkippedSupplements(false);
  } else {
    setSelectedSupplements([]);
    setSkippedSupplements(true);
  }

  // Load container
  if (cart.container?.length > 0) {
    const contName = cart.container[0].name;
    const contIdx = containers.findIndex(c => c.name === contName);
    setSelectedContainer(contIdx !== -1 ? contIdx : null);
  } else {
    setSelectedContainer(null);
  }

  // Load frequency
  if (cart.frequency?.length > 0) {
    const freqName = cart.frequency[0].name;
    const freqOption = frequencyOptions.find(f => f.name === freqName);
    setSelectedFrequency(freqOption ? freqOption.value : null);
  } else {
